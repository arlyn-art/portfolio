export default {
    hero: {
        badge: "Available for Freelance",
        greeting: "Hi, I'm",
        name: "Arlyn",
        job: "Full Stack Developer",
        description:
            "Saya membangun website modern yang cepat, scalable, dan mudah digunakan.",

        buttons: {
            cv: "Lihat CV",
            portfolio: "Lihat Portofolio"
        },

        stats: [
            {
                value: "4+",
                label: "Tahun Pengalaman"
            },
            {
                value: "20+",
                label: "Project"
            },
            {
                value: "100%",
                label: "Kepuasan Klien"
            },
            {
                value: "20+",
                label: "Klien & Mitra"
            }
        ]
    },
    secondary: {
        title: "JELAJAHI",
        subtitle: "Membangun Solusi Digital",
        description: "Dari arsitektur backend hingga antarmuka web modern, berikut karya dan pengalaman yang telah saya bangun.",

        button_label: ["Semua", "Tinjauan", "Pengalaman", "Project", "Kontak"]
    },

    overview: {
        title: "OVERVIEW",
        subtitle: "Membangun aplikasi web yang modern, cepat, dan scalable.",
        description:
            "Saya adalah Full Stack Web Developer dengan pengalaman lebih dari 3 tahun dalam membangun aplikasi web menggunakan Laravel, Vue.js, React, Filament, dan Tailwind CSS.",

        cards: [
            {
                title: "Full Stack Development",
                description:
                    "Mengembangkan aplikasi web end-to-end menggunakan Laravel, REST API, dan database relasional yang aman serta mudah dikembangkan."
            },
            {
                title: "Modern Interface",
                description:
                    "Membangun antarmuka yang responsif, bersih, dan nyaman digunakan menggunakan Blade, Vue.js, React, dan Tailwind CSS."
            },
            {
                title: "System Architecture",
                description:
                    "Merancang struktur database, alur sistem, deployment server, debugging, dan optimasi performa aplikasi."
            }
        ],

        tech_stack: "Tech Stack"
    },

    experience: {
        title: "PENGALAMAN",
        subtitle: "Perjalanan Profesional",
        description:
            "Pengalaman profesional yang membentuk kemampuan saya dalam membangun aplikasi web modern.",

        current: {
            badge: "Posisi Saat Ini",
            position: "Website Programmer",
            company: "PT Design Tiga Selaras",
            period: "Nov 2024 — Sekarang",
            description:
                "Mengembangkan website perusahaan, CMS, REST API, database, deployment, dan optimasi performa menggunakan ekosistem Laravel."
        },

        freelance: {
            title: "Freelance Aktif",
            position: "Full Stack Developer",
            company: "Self-Employed",
            period: "Sep 2023 — Sekarang",
            description:
                "Membangun website, CMS, REST API, dan sistem internal sesuai kebutuhan berbagai klien."
        },

        previous: {
            title: "Pengalaman Sebelumnya",
            items: [
                {
                    company: "PT Krakatau Information Technology",
                    period: "Apr 2022 — Jun 2023",
                    roles: [

                        {
                            period: "Apr 2022 — Jun 2022",
                            position: "Customer Services",
                            description:
                                "Melanjutkan dukungan pada proyek Satgas COVID-19 setelah kontrak layanan berpindah dari vendor sebelumnya ke PT Krakatau Information Technology."
                        },
                        {
                            period: "Jul 2022 — Jun 2023",
                            position: "Technical Writer",
                            description:
                                "Melanjutkan kontrak baru sebagai Technical Writer dengan menyusun dokumentasi teknis, SOP, dan berkolaborasi dengan berbagai tim untuk mendukung kebutuhan operasional."
                        }

                    ]
                },
                {
                    position: "Supervisor Helpdesk",
                    company: "PT Ivosights",
                    period: "Jan 2021 — Mar 2022",
                    description:
                        "Memimpin operasional helpdesk pada proyek Satgas COVID-19, mengoordinasikan layanan dan penyelesaian masalah hingga kontrak layanan dialihkan ke penyedia berikutnya."
                }
            ]
        }
    },

    projects:{
        title:"PROJECTS",
        subtitle:"Karya Terpilih",
        description:"Berikut beberapa proyek yang pernah saya kerjakan.",
        tabs:[
            "Komersial",
            "Pribadi",
            "Dokumentasi"
        ],
        button:"Lihat Semua Proyek"
    },

    channels:{
        title:"👋 Hubungi Saya Melalui<br>Media Sosial",
        description:"Pilih saluran komunikasi yang ingin digunakan untuk menghubungi saya.",
    },

    footer: {
        "about": "Membantu merealisasikan ide digital menjadi kenyataan melalui kode yang bersih dan desain yang interaktif.",
        "nav_title": "Navigasi",
        "connect_title": "Mari Terhubung",
        "connect_desc": "Tertarik untuk berkolaborasi? Hubungi saya kapan saja.",
        "built_with": "Dibuat dengan menggunakan Tailwind & Alpine"
    },

    profile: {
        title: "PROFIL",
        subtitle: "Tentang Saya",
        description:
            "Mengenal lebih dekat perjalanan, pola pikir, dan pengalaman saya sebagai Full Stack Web Developer.",
        badge: "Tentang Saya",
        about_title: "Membangun solusi digital yang sederhana, cepat, dan mudah digunakan.",
        about_1:
            "Saya adalah Full Stack Web Developer dengan pengalaman lebih dari 3 tahun membangun website perusahaan, dashboard CMS, REST API, dan sistem internal menggunakan Laravel, Vue.js, React, Filament, serta Tailwind CSS.",
        about_2:
            "Saya menikmati proses mengubah kebutuhan bisnis menjadi solusi digital yang efisien. Saya percaya bahwa software yang baik bukan hanya bekerja dengan benar, tetapi juga mudah dipelihara, cepat digunakan, dan memberikan pengalaman terbaik bagi pengguna.",
        facts: [
            {
                title: "Lokasi",
                value: "Indonesia"
            },
            {
                title: "Pengalaman",
                value: "4+ Tahun"
            },
            {
                title: "Fokus",
                value: "Full Stack Web"
            },
            {
                title: "Status",
                value: "Open for Freelance"
            }
        ],

        values_title: "Prinsip yang Saya Pegang",
        values: [
            {
                title: "Clean Code",
                description:
                    "Menulis kode yang mudah dipahami, mudah dikembangkan, dan memiliki struktur yang konsisten."
            },
            {
                title: "Problem Solving",
                description:
                    "Fokus memahami masalah bisnis terlebih dahulu sebelum menentukan solusi teknis."
            },
            {
                title: "Continuous Learning",
                description:
                    "Selalu mempelajari teknologi baru agar dapat membangun aplikasi yang lebih baik setiap harinya."
            }
        ]
    },

    journey:{
        title:"PERJALANAN",
        subtitle:"Hal-Hal yang Membentuk Saya di Luar Dunia Coding",
        description:"Di luar dunia pemrograman, saya senang menjelajahi tempat baru, mendaki gunung, menikmati budaya lokal, dan mengabadikan berbagai momen perjalanan. Setiap perjalanan memberikan pelajaran yang juga memengaruhi cara saya bekerja dan menyelesaikan masalah.",
        quote:
        "Setiap tempat yang saya kunjungi selalu meninggalkan pelajaran yang membentuk cara saya berpikir, berkarya, dan menyelesaikan masalah.",
        travel_title:"Tempat yang Pernah Saya Jelajahi",
        travel_description:"Kumpulan perjalanan yang menjadi bagian dari cerita hidup saya.",
        lessons_title:"Pelajaran dari Setiap Perjalanan",
        lessons:[
            {
                title:"Mudah Beradaptasi",
                description:"Setiap tempat memiliki tantangan yang berbeda. Hal ini membuat saya lebih cepat beradaptasi ketika menghadapi masalah teknis."
            },
            {
                title:"Kesabaran",
                description:"Pendakian panjang, cuaca yang berubah, dan perjalanan jauh mengajarkan bahwa hasil terbaik membutuhkan proses."
            },
            {
                title:"Rasa Ingin Tahu",
                description:"Menjelajahi tempat dan budaya baru membuat saya selalu terdorong untuk terus belajar, baik dalam teknologi maupun kehidupan."
            }
        ]

    }
}